...
#include <dolphin/internal/__gx.h>
#include <dolphin/internal/__os.h>
#include <dolphin/internal/__vi.h>
...
void VIConfigure(GXRenderModeObj *rm) {
  ...
  if (rm->xFBmode == VI_XFBMODE_SF && rm->viTVmode != VI_TVMODE_NTSC_PROG) {
    ASSERTMSGLINEV(
        0x60E, rm->viHeight == rm->xfbHeight * 2,
  ...
  HorVer.DispSizeY =
      (u16)((HorVer.nonInter == VI_PROGRESSIVE) ? HorVer.PanSizeY
            : (HorVer.FBMode == VI_XFBMODE_SF)  ? (u16)(2 * HorVer.PanSizeY)
                                                : HorVer.PanSizeY);
  ...
  ASSERTMSGLINEV(0x659, rm->viXOrigin + rm->viWidth >= 0x2A8 - tm->hbs640,
                 "VIConfigure(): viXOrigin + viWidth (%d) cannot be less than "
                 "%d in this TV mode\n",
  ...
  if ((HorVer.nonInter == VI_PROGRESSIVE)) {
    ...
    regDspCfg = (((u32)(regDspCfg)) & ~0x00000004) |
                (((u32)(HorVer.nonInter & 1)) << 2);
  }
